...
static double bufOut[2][7];
static double bufIn[2][7];

static double bufOut_Ref[2][7];
static double bufIn_Ref[2][7];

static double bufOut_Fin[2][4];
static double bufIn_Fin[2][4];

//==============================================================================
/**
 */
class VirtualTubeFlangerAudioProcessor  : public AudioProcessor
{
public:
    //==============================================================================
    VirtualTubeFlangerAudioProcessor();
    ~VirtualTubeFlangerAudioProcessor();
    ...
    float* delayTimeLeft_ = nullptr; // Tube length in m
    float* delayTimeRight_ = nullptr;
    ...
    float* delayTimeRefLeft_ = nullptr; // Tube length in m
    float* delayTimeRefRight_ = nullptr;
    ...
    float delayMilli[2];
    float delayMilliRef[2];

    
    Filter mFilter[2];
    DelayLine mDelayLine[2];
    
    Filter mFilter_ref[2];
    DelayLine mDelayLine_ref[2];
    
    AudioProcessorValueTreeState parameters;
    
    void applyGainAndWidth(int numSamples);
    void applyFilterDelay(int numSamples, int channel);
    
    double computeOutFilter(double input, int channel);
    double computeOutReflectionFilter(double input, int rad, int channel);
...
private:
    ...
    float maxDelaySamples[2];
    float maxDelaySamplesRef[2];
    
    float currentWidth[2];
    float previousWidth[2];
    float interpolatedWidth[2];
    float incrementWidth[2];
    
    float currentGain[2];
    float currentGainRef[2];
    
    float previousGain[2];
    float interpolatedGain[2];
    float incrementGain[2];
    
    float previousGainRef[2];
    float interpolatedGainRef[2];
    float incrementGainRef[2];
    
    float previousMaxDelaySamples[2];
    float interpolatedMaxDelaySamples[2];
    float incrementMaxDelaySamples[2];
    
    float previousMaxDelaySamplesRef[2];
    float interpolatedMaxDelaySamplesRef[2];
    float incrementMaxDelaySamplesRef[2];
    
    float currentDelay[2];
    float currentDelayRef[2];
    float currentLength[2];
    float currentLengthRef[2];
    ...
    //==============================================================================
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VirtualTubeFlangerAudioProcessor)
};
