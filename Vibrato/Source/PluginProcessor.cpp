...
//==============================================================================
VirtualTubeVibratoAudioProcessor::VirtualTubeVibratoAudioProcessor()
...
#endif
{
    ...
    parameters.createAndAddParameter("delayTimeLeft_", "delayTimeLeft", "delayTimeLeft", NormalisableRange<float>(3.0f,10.0f), 3.0f, nullptr, nullptr);
    parameters.createAndAddParameter("delayTimeRight_", "delayTimeRight", "delayTimeRight", NormalisableRange<float>(3.0f,10.0f), 3.0f, nullptr, nullptr);
    ...
    parameters.createAndAddParameter("delayTimeRefLeft_", "delayTimeRefLeft", "delayTimeRefLeft", NormalisableRange<float>(3.0f,10.0f), 3.0f, nullptr, nullptr);
    parameters.createAndAddParameter("delayTimeRefRight_", "delayTimeRefRight", "delayTimeRefRight", NormalisableRange<float>(3.0f,10.0f), 3.0f, nullptr, nullptr);
    ...
    parameters.createAndAddParameter("frequencyRate_", "frequencyRate", "frequencyRate", NormalisableRange<float>(2.0f, 14.0f), 2.0f, nullptr, nullptr);
...
//==============================================================================
const String VirtualTubeVibratoAudioProcessor::getName() const
{
    return JucePlugin_Name;
}

bool VirtualTubeVibratoAudioProcessor::acceptsMidi() const
{
...
bool VirtualTubeVibratoAudioProcessor::producesMidi() const
{
...
bool VirtualTubeVibratoAudioProcessor::isMidiEffect() const
{
   #if JucePlugin_IsMidiEffect
    return true;
   #else
...
double VirtualTubeVibratoAudioProcessor::getTailLengthSeconds() const
{
    return 0.0;
}

int VirtualTubeVibratoAudioProcessor::getNumPrograms()
{
    return 1;   // NB: some hosts don't cope very well if you tell them there are 0 programs,
                // so this should be at least 1, even if you're not really implementing programs.
}

int VirtualTubeVibratoAudioProcessor::getCurrentProgram()
{
    return 0;
}

void VirtualTubeVibratoAudioProcessor::setCurrentProgram (int index)
{
}

const String VirtualTubeVibratoAudioProcessor::getProgramName (int index)
{
    return {};
}

void VirtualTubeVibratoAudioProcessor::changeProgramName (int index, const String& newName)
{
}

//==============================================================================
void VirtualTubeVibratoAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    for(int channel = 0; channel < getTotalNumOutputChannels(); ++channel)
    {
        previousMaxDelaySamples[channel] = maxDelaySamples[channel] = 132.3f;
        previousMaxDelaySamplesRef[channel] = maxDelaySamplesRef[channel] = 132.3f;
...
void VirtualTubeVibratoAudioProcessor::releaseResources()
{
    for(int channel = 0; channel < getTotalNumOutputChannels(); ++channel)
    {
        mDelayLine[channel].suspend();
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool VirtualTubeVibratoAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
  #if JucePlugin_IsMidiEffect
...
void VirtualTubeVibratoAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < 2; ++channel)
    {
        ...
        maxDelaySamplesRef[channel] = maxDelaySamples[channel] + 2.0f*((delayMilliRef[channel])*(sampleRate/1000.0f));
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            currentDelay[channel] = interpolatedMaxDelaySamples[channel]/2.0f * ((1 + sin(2.0f * M_PI * ph[channel]) * interpolatedWidth[channel]));
            ...
            double out1 = currentGain[channel] * mDelayLine[channel].delayLine(x, static_cast<double>(currentDelay[channel]), channel);
            
            double out2 = currentGainRef[channel] * mDelayLine_ref[channel].delayLine(x_fin, static_cast<double>(currentDelayRef[channel]), channel);
...
void VirtualTubeVibratoAudioProcessor::process(int channel) {
...
double VirtualTubeVibratoAudioProcessor::computeOutFilter(double input, int channel){
...
double VirtualTubeVibratoAudioProcessor::computeOutReflectionFilter(double input, int rad, int channel){
...
void VirtualTubeVibratoAudioProcessor::applyFilterDelay(int numSamples, int channel){
...
void VirtualTubeVibratoAudioProcessor::applyGainAndWidth(int numSamples){
...
//==============================================================================
bool VirtualTubeVibratoAudioProcessor::hasEditor() const
{
    return true; // (change this to false if you choose to not supply an editor)
}

AudioProcessorEditor* VirtualTubeVibratoAudioProcessor::createEditor()
{
    return new VirtualTubeVibratoAudioProcessorEditor (*this);
}

//==============================================================================
void VirtualTubeVibratoAudioProcessor::getStateInformation (MemoryBlock& destData)
{
    ScopedPointer<XmlElement> xml (parameters.state.createXml());
    copyXmlToBinary (*xml, destData);
}

void VirtualTubeVibratoAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new VirtualTubeVibratoAudioProcessor();
}
