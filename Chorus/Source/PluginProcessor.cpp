...
#endif
{
    ...
    parameters.createAndAddParameter("gainLeft_", "gainLeft", "gainLeft", NormalisableRange<float>(0.0, 3.0), 0.0, nullptr, nullptr);
    parameters.createAndAddParameter("gainRight_", "gainRight", "gainRight", NormalisableRange<float>(0.0, 3.0), 0.0, nullptr, nullptr);
    ...
    parameters.createAndAddParameter("gainRefLeft_", "gainRefLeft", "gainRefLeft", NormalisableRange<float>(0.0, 9.0), 0.0, nullptr, nullptr);
    parameters.createAndAddParameter("gainRefRight_", "gainRefRight", "gainRefRight", NormalisableRange<float>(0.0, 9.0), 0.0, nullptr, nullptr);
...
void VirtualTubeChorusAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    previousGain_L = 0.0;
    previousGain_R = 0.0;
    previousGainRef_L = 0.0;
    previousGainRef_R = 0.0;
...
void VirtualTubeChorusAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        outL = inL + (wetValue_) * (currentGain_L * mDelayLine.delayLine_Chorus_L(xL * feedback, currentDelayL) + currentGainRef_L * mDelayLine.delayLine_Chorus_Ref_L(x_fin_L * feedback, currentDelayRefL));
        
        outR = inR + (wetValue_) * (currentGain_R * mDelayLine.delayLine_Chorus_R(xR * feedback, currentDelayR) + currentGainRef_R * mDelayLine.delayLine_Chorus_Ref_R(x_fin_R * feedback, currentDelayRefR));
...
void VirtualTubeChorusAudioProcessor::applyFilterDelay_L(int numSamples){
...
void VirtualTubeChorusAudioProcessor::applyFilterDelay_R(int numSamples){
...
void VirtualTubeChorusAudioProcessor::applyGain(int numSamples){
...
void VirtualTubeChorusAudioProcessor::process() {
...
double VirtualTubeChorusAudioProcessor::computeOutFilter_L(double input){
...
double VirtualTubeChorusAudioProcessor::computeOutFilter_R(double input){
...
double VirtualTubeChorusAudioProcessor::computeOutReflectionFilter_L(double input){
...
double VirtualTubeChorusAudioProcessor::computeOutReflectionFilter_R(double input){
